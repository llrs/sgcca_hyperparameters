...
```{r}
library("broom")
lmC0 <- lm(AVE_inner~0+var12+var13+var14+var23+var24+var34, data = def)
lmC <- lm(AVE_inner~0+var12*var13*var14*var23*var24*var34, data = def)
anova(lmC, lmC0)
glance(lmC)
tidy(lmC) %>% arrange(desc(abs(estimate)))

# Add the information we know 
# (If weights equal 0, there is no information of the model)
empty <- rep(0, ncol(def))
names(empty) <- colnames(def)
def <- rbind(def, empty)
lmC0 <- lm(AVE_inner~0+var12+var13+var14+var23+var24+var34, data = def)
lmC <- lm(AVE_inner~0+var12*var13*var14*var23*var24*var34, data = def)
anova(lmC, lmC0)
