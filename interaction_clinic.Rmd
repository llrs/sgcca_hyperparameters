...
```{r}
out <- sapply(C_list3, testing, type = "centroid")
out2 <- t(out)
...
```{r}
library("broom")
lmIC0 <- lm(AVE_inner~0+var12+var13+var23+var14+var23+var34+var11, data = def)
lmIC <- lm(AVE_inner~0+var12*var13*var23*var14*var23*var34*var11, data = def)
anova(lmIC, lmIC0)
glance(lmIC)
tidy(lmIC) %>% arrange(desc(estimate))
