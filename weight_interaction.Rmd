...
testing <- function(x, type) {
  ...
  type <- match.arg(type, c("centroid", "horst", "factorial"))
...
```{r}
library("broom")
lmI0 <- lm(AVE_inner ~0+var12+var13+var23+var11, data = def) 
lmI <- lm(AVE_inner ~0+var12*var13*var23*var11, data = def) 
anova(lmI, lmI0)
glance(lmI)
tidy(lmI) %>% arrange(desc(abs(estimate)))
...
saveRDS(boot, file = "bootstrap_model2.RDS") # Save
