...
sgcca.final = sgcca(B, D, c1 = c(.071,.2, 1/sqrt(NCOL(B[[3]]))+.2, 1),
    ...
    scheme = "centroid", scale = TRUE,
    verbose = FALSE)
final.superblock <- cbind.data.frame(sgcca.final$Y[[3]], Loc, Samples = rownames(sgcca.glioma$Y[[3]]))
final.GE <- cbind.data.frame(sgcca.final$Y[[1]], Loc, Samples = rownames(sgcca.glioma$Y[[3]]))
